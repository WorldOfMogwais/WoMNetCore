using System.Collections.Generic;
using System.Linq;
using WoMFramework.Game.Interaction;
using WoMFramework.Game.Model;
using WoMFramework.Game.Model.Mogwai;
using WoMFramework.Tool;
using Xunit;

namespace WoMFrameworkTest.Game
{
    public class HomeTownTest
    {
        [Fact]
        public void ShopTest()
        {
            var address = "MJHYMxu2kyR1Bi4pYwktbeCM7yjZyVxt2i";
            var pubMogAddressHex = HexHashUtil.ByteArrayToString(Base58Encoding.Decode(address));

            var creation = new Shift(0, 1530914381, pubMogAddressHex,
                2000, "00000000090d6c6b058227bb61ca2915a84998703d4444cc2641e6a0da4ba37e",
                2, "163d2e383c77765232be1d9ed5e06749a814de49b4c0a8aebf324c0e9e2fd1cf",
                1.00m,
                0.0001m);
            var shifts = new Dictionary<double, Shift>
            {
                {2000, creation}
            };
            var mogwai = new Mogwai(address, shifts);

            Assert.Equal(9, mogwai.HomeTown.Shop.Inventory.Count);
            Assert.True(mogwai.HomeTown.Shop.Inventory.Sum(p => p.Cost) < 50);
            Assert.True(mogwai.HomeTown.Shop.Inventory.OfType<Armor>().Count() > 0);
            Assert.True(mogwai.HomeTown.Shop.Inventory.OfType<Weapon>().Count() > 0);
        }
    }
}
